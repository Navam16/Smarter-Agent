...
def make_candidate_id(github_url: str = None, email: str = None, name: str = "", company: str = "") -> str:
    ...
    if existing:
        ...
        conn.execute("""
            UPDATE candidates SET
                ...
                activity_score = COALESCE(NULLIF(?, ''), activity_score)
            WHERE id = ?
        """, (
            now,
            data.get("role", ""),
        ...
        conn.execute("""
            INSERT INTO candidates (
                ...
                activity_score, job_seeking, status, created_at, last_seen_at, raw_data
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            cid,
...
def get_all_candidates(status: str = None, source: str = None, limit: int = 500) -> list[dict]:
...
def search_candidates(query: str) -> list[dict]:
    ...
    rows = conn.execute("""
        ...
        WHERE name LIKE ? OR role LIKE ? OR skills LIKE ?
           OR company LIKE ? OR location LIKE ? OR bio LIKE ?
