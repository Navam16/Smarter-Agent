...
def make_candidate_id(
    ...
    """Generate unique fingerprint for candidate. Safely handles None values."""
    ...
    if github_url:
        ...
        key = f"{name.lower()}_{company.lower()}"
    else:
        # Last resort — random-ish key using timestamp
        key = f"unknown_{datetime.utcnow().isoformat()}"
